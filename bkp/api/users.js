        ...
        if (req.method === 'POST') {
            ...
            form.parse(req, async (err, fields, files) => {
                ...
                if (user_id) {
                    ...
                    // Verifica se o arquivo e o mimetype existem e não são nulos.
                    if (photoFile && photoFile.size > 0 && photoFile.mimetype) {
                        const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif'];
                        const maxSize = 5 * 1024 * 1024; // 5 MB

                        if (!allowedTypes.includes(photoFile.mimetype) || photoFile.size > maxSize) {
                            return res.status(400).json({ message: `Tipo de arquivo inválido ou muito grande. Tipo recebido: ${photoFile.mimetype}. Apenas imagens JPEG, JPG, PNG ou GIF até 5MB são permitidas.` });
                        }
