...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NOVAS VARIÁVEIS PARA AS GALERIAS
    const attentionPointsGrid = document.getElementById('attention-points-grid');
    ...
    // NOVOS BOTÕES
    const attentionPointsButton = document.getElementById('attentionPointsButtonCollapse');
    const treatmentsButton = document.getElementById('treatmentsButtonCollapse');
    ...
    // A função renderPhotos foi generalizada para usar campos de data de forma dinâmica
    function renderPhotos(records, gridElement, photoUrlKey) {
        ...
        records.forEach(record => {
            if (record[photoUrlKey]) {
                ...
                // Determinando o campo de data a ser exibido
                const dateToShow = record.record_date || record.created_at || record.treatment_date;
                const formattedDate = dateToShow ? new Date(dateToShow).toLocaleDateString('pt-BR') : '';
    ...
    async function loadUserDataAndRecords() {
        try {
            ...
            if (response.ok) {
                ...
                // Chamando a mesma função para as novas galerias
                ...
                const hasAttentionPoints = renderPhotos(userData.attention_points || [], attentionPointsGrid, 'photo_url');
                const hasTreatments = renderPhotos(userData.treatments || [], treatmentsGrid, 'photo_url');
                ...
                if (attentionPointsButton) {
                    if (hasAttentionPoints) {
                        attentionPointsButton.classList.add('btn-with-photos');
                        attentionPointsButton.classList.remove('btn-no-photos');
                    } else {
                        attentionPointsButton.classList.add('btn-no-photos');
                        attentionPointsButton.classList.remove('btn-with-photos');
                    }
                }
                if (treatmentsButton) {
                    if (hasTreatments) {
                        treatmentsButton.classList.add('btn-with-photos');
                        treatmentsButton.classList.remove('btn-no-photos');
                    } else {
                        treatmentsButton.classList.add('btn-no-photos');
                        treatmentsButton.classList.remove('btn-with-photos');
                    }
                }
            } else {
