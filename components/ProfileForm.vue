// /components/ProfileForm.vue - V4.1 - Inclusão do campo 'telefone'
<template>
...
const props = defineProps<{
  initialData: UserProfileData; // Usando a interface tipada
  isEdit: boolean;
}>();
...
const formData = reactive({
  ...
  telefone: props.initialData.telefone || '', // Adicionado telefone
});
...
watch(() => props.initialData, (newVal) => {
  ...
  formData.sexo = newVal.sexo || 'O';
    formData.telefone = newVal.telefone || ''; // Sincroniza telefone
  ...
  try {
    ...
    const response = await fetch('/api/profile', { 
      ...
      body: JSON.stringify({
        ...
        sexo: formData.sexo,
                telefone: formData.telefone || null, // Inclui telefone no payload
      }),
    });
