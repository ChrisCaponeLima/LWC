// /components/ProfileForm.vue - V4.3 - Remoção da funcionalidade de exclusão de foto de perfil (botão e lógica 'removePhoto')
<template>
    ...
    <div v-if="photoSubmissionSuccess" class="p-2 bg-green-100 text-green-700 border border-green-400 rounded-md w-full text-sm text-center">
     {{ photoSubmissionSuccess }}
      ...
      <div class="flex flex-row gap-2 mt-2 w-full">
          ...
          :class="[
            'px-6 py-2 rounded-md font-bold transition duration-150 w-full', // Ajuste para w-full
            {
              'bg-blue-500 text-white hover:bg-blue-600': selectedFile,
              'bg-gray-300 text-gray-500 cursor-not-allowed': !selectedFile,
              // A classe 'flex-1' foi substituída por 'w-full' para ocupar o espaço total
            }
          ]"
        >
    ...
    // 🛑 REMOÇÃO V4.3: A lógica de Remoção de Foto (função removePhoto) foi removida a pedido do usuário.
    /*
