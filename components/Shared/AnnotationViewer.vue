// /components/Shared/AnnotationViewer.vue - V1.1 - Adaptação para receber objeto JSON já parseado do servidor
<template>
  <div 
...
// /components/Shared/SharedAnnotationViewer.vue - V1.1 - Adaptação para receber objeto JSON já parseado do servidor
...
const props = defineProps<{
  imageUrl: string;
    // 🟢 ALTERAÇÃO: Agora esperamos um objeto (o JSON já parseado) ou null.
  annotationDataJson: AnnotationData | null; 
}>();
...
// 🟢 ALTERAÇÃO: Recebe AnnotationData | null. Não faz mais o JSON.parse.
const initializeAnnotations = (data: AnnotationData | null) => {
  if (data) {
    try {
            // Apenas atribui o objeto, sem tentar parsear
      localAnnotations.value = {
        drawings: data.drawings || [],
        texts: data.texts || [],
      };
    } catch (e) {
      // Este catch serve mais como um fallback de segurança
...
// O watcher continua funcionando, mas a função initializeAnnotations agora recebe um objeto.
