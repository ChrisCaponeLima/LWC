// /components/Treatments/EndTreatmentModal.vue - V1.2 - CORREÇÃO CRÍTICA: Declarando a prop 'userId' para evitar 'undefined' na rota da API.
<template>
...
const props = defineProps<{
 ...
 userId: number; // ⬅️ CORREÇÃO CRÍTICA: Declaração do userId
}>();
...
const submitEndTreatment = async () => {
 if (!isValid.value || !props.treatmentToEnd || !props.userId) { // Adicionado check para userId
  errorMessage.value = "Dados de usuário ou tratamento ausentes.";
  return;
 }
...
// Usando o método seguro de formatação de data
