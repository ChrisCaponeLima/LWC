// /composables/useKpiData.ts - V1.12 - Adiciona interface FileRecord e o campo all_files na interface Record para suportar a galeria agregada do DataDisplay.vue.
...
export function useKpiData() {
 ...
 const fetchData = async () => {
  ...
  try {
   ...
   // NOVO: Interface para o arquivo (esperado pelo DataDisplay)
   ...
   interface Record {
    ...
    // CRÍTICO: Novo campo para suportar o DataDisplay
   ...
   if (records && records.length > 0) {
    ...
    // A lógica de has...Photos deve ser ajustada para usar 'all_files' se disponível, 
    // mas manteremos a lógica antiga como fallback/para simplificar a compatibilidade
    hasRegistroPhotos.value = records.some(record => {
     return !!record.photo_url || record.all_files?.some(f => f.type === 'registro')
    })
    hasFormaPhotos.value = records.some(record => {
     return !!record.forma_url || record.all_files?.some(f => f.type === 'forma')
    })

    // Garante que o DataDisplay receba os records com o campo all_files
