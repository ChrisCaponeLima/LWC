// /pages/ocr/peso-balanca.vue - V25 - Lógica Limpa (Backend V34 usa PSM 6 no binarizado)
// O código é idêntico ao V24, pois a lógica de extração não muda.

<template>
  <div class="p-6 max-w-3xl mx-auto">
    <h1 class="text-2xl font-bold mb-4 text-center">Leitura de Peso via OCR</h1>

    <div class="bg-white shadow-md rounded-2xl p-6 space-y-6">
      <div class="flex flex-col items-center space-y-4">
        <label
          for="file"
          class="cursor-pointer bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition"
        >
          Enviar imagem da balança
        </label>
        <input
          id="file"
          type="file"
          accept="image/*"
          class="hidden"
          @change="onFileChange"
        />

        <div v-if="previewUrl" class="relative mt-4">
          <img
            :src="previewUrl"
            alt="Pré-visualização"
            class="rounded-lg shadow-md max-h-80 object-contain"
          />
        </div>
      </div>

      <div v-if="isLoading" class="text-center text-gray-600">Processando OCR...</div>

      <div v-if="ocrRaw || ocrResult || ocrError" class="space-y-4">
        <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <p class="font-semibold text-gray-700 mb-1">Texto completo lido pelo OCR:</p>
          <textarea
            v-model="ocrRaw"
            readonly
            class="w-full border rounded-lg p-2 text-sm font-mono bg-white"
            rows="8"
          ></textarea>
        </div>

        <div v-if="ocrResult" class="p-4 bg-green-50 border border-green-200 rounded-lg">
          <p class="font-semibold text-green-700">Peso Detectado:</p>
          <p class="text-xl font-bold text-green-800">{{ ocrResult }}</p>
        </div>

        <div v-if="ocrError" class="p-4 bg-red-50 border border-red-200 rounded-lg">
          <p class="font-semibold text-red-700">Erro:</p>
          <p class="text-sm text-red-600">{{ ocrError }}</p>
        </div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref } from 'vue'

const previewUrl = ref<string | null>(null)
const selectedFile = ref<File | null>(null)
const isLoading = ref(false)
const ocrRaw = ref<string>('')
const ocrResult = ref<string | null>(null)
const ocrError = ref<string | null>(null)

const MIN_WEIGHT = 30.0 
const MAX_WEIGHT = 200.0 

function onFileChange(e: Event) {
  const file = (e.target as HTMLInputElement).files?.[0]
  if (!file) return

  selectedFile.value = file
  previewUrl.value = URL.createObjectURL(file)
  ocrRaw.value = ''
  ocrResult.value = null
  ocrError.value = null

  setTimeout(() => {
    performOcr()
  }, 100) 
}

async function performOcr() {
  if (!selectedFile.value) return
  isLoading.value = true
  ocrRaw.value = ''
  ocrResult.value = null
  ocrError.value = null

  try {
    const formData = new FormData()
    formData.append('image', selectedFile.value)

    const res = await fetch('/api/ocr/peso-balanca', {
      method: 'POST',
      body: formData
    })
    const data = await res.json()
    
    if (!res.ok) {
        throw new Error(data?.statusMessage || data?.error || 'Erro desconhecido no servidor.')
    }

    ocrRaw.value = data?.text ?? ''

    const cleanedText = ocrRaw.value.replace(/(\r\n|\n|\r)/gm, ' ').trim()
    
    // 1. Tenta encontrar o padrão decimal (X.X, XX.X, XXX.X)
    const match = cleanedText.match(/(\d{1,3}[.,]\d{1})/); 
    
    let finalWeight: number | null = null;

    if (match) {
        // Usa o decimal encontrado
        const value = parseFloat(match[0].replace(',', '.')) 
        
        if (value >= MIN_WEIGHT && value <= MAX_WEIGHT) {
            finalWeight = value
        }
    } else {
        // 2. Se falhar no decimal, tenta encontrar o maior inteiro válido
        const simpleMatches = cleanedText.match(/\d+/g)
        if (simpleMatches) {
            const integerCandidates = simpleMatches
                .map(s => parseFloat(s))
                .filter(v => v >= MIN_WEIGHT && v <= MAX_WEIGHT)

            if (integerCandidates.length > 0) {
                 // Pega o maior inteiro como peso 
                 finalWeight = Math.max(...integerCandidates)
            }
        }
    }


    if (finalWeight !== null) {
      // Garante uma casa decimal no output para padronização
      ocrResult.value = finalWeight.toFixed(1) + ' kg' 
    } else {
      ocrResult.value = null
      ocrError.value = `Nenhum peso válido encontrado. Texto cru: "${cleanedText}"`
    }

  } catch (err: any) {
    ocrError.value = err.message || 'Falha ao processar imagem.'
  } finally {
    isLoading.value = false
  }
}
</script>

<style scoped>
textarea { resize: vertical; }
</style>