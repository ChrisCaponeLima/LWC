// /pages/professional/treatments/[id].vue - V4.1 - Padronização: Adicionadas importações explícitas de Header e Footer para rastreabilidade e estabilidade.
<template>
<div>
<Header :pageTitle="`Tratamentos - ${data?.targetUser.name || 'Carregando...'}`" />
...
:user-treatments="data.activeTreatments" 
:available-treatments="data.availableTreatments" 
@evaluation-saved="handleAssessmentUploadSuccess"
@open-measurements-form="openMeasurementsModal"
/>
</div>
...
<h3 class="text-xl font-semibold mb-4 text-gray-700">Gerenciar Tratamentos</h3>
<TreatmentAssociationManager 
:user-id="targetUserId"
:active-treatments="data.activeTreatments"
:available-treatments="data.availableTreatments"
:available-areas="data.availableAreas" @refreshData="fetchData" 
/>
</div>
</div>
</div>
...
<div v-if="isMeasurementsModalOpen" class="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center p-4">
 <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto relative">
  <button 
   @click="closeMeasurementsModal"
   class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 z-10 p-2"
  >
   <i class="fas fa-times text-xl"></i>
  </button>
  <div class="p-6">
   <h3 class="text-2xl font-bold mb-4 text-gray-800 border-b pb-2">
    <i class="fas fa-ruler-vertical mr-2 text-blue-600"></i> Registrar Medidas
   </h3>
   
   <DataFormV2 
    :user-id="targetUserId"
    :patient-name="data.targetUser.name" 
    @record-saved="handleMeasurementsSaved"
    @cancel="closeMeasurementsModal"
   />
  </div>
 </div>
</div>
...
import Header from '~/components/Header.vue' // ✅ Adicionado explicitamente
import Footer from '~/components/Footer.vue' // ✅ Adicionado explicitamente
...
// Adicionando DataFormV2 para a inserção de medidas
import DataFormV2 from '~/components/DataFormV2.vue' 
// =================================
...
// Estado para o Modal de Medidas (NOVO)
const isMeasurementsModalOpen = ref(false)

// Inicializa a aba de Avaliação como padrão
const activeTab = ref('assessment') 
const galleryRefreshTrigger = ref(0);

const tabs = [
{ name: 'assessment', label: 'Avaliação' },
{ name: 'activeTreatments', label: 'Tratamentos' },
{ name: 'images', label: 'Galerias de Imagens' }, 
]

// -----------------------------------------------------------
// LÓGICA DO MODAL DE MEDIDAS (NOVAS FUNÇÕES)
// -----------------------------------------------------------
const openMeasurementsModal = () => {
 console.log(`[Page] Abrindo modal de medidas para o usuário ID: ${targetUserId}`);
...
/**
* Lida com o evento de sucesso após salvar as medidas.
* Recarrega os dados da página para atualizar quaisquer métricas ou histórico.
*/
const handleMeasurementsSaved = () => {
 closeMeasurementsModal();
 // Força o recarregamento dos dados da página para refletir as novas medidas
 fetchData(); 
...
// -----------------------------------------------------------
// FUNÇÃO DE BUSCA DE DADOS (Mantida)
// -----------------------------------------------------------
