// /pages/registro/imagem.vue - V1.9 - Inclusão do botão de download da imagem editada.
<template>
<NuxtLayout>
 <div>
  ...
  <div v-else>
   ...
   <ClientOnly>
    <ImageEditorComponent 
     ref="imageEditorRef"     :initial-image-url="editingFileUrl"
...
// 🚨 NOVO: Ref para acessar métodos do ImageEditorComponent
const imageEditorRef = ref(null); 

// Estado de Edição
const isEditing = ref(false);
const editingFile = ref(null); 
const editingFileUrl = ref(null); 
...
const loadTempFilesFromSession = () => {
 if (process.client) {
  ...
  tempFormaFiles.value = formaDataRaw ? JSON.parse(formaDataRaw) : [];
 }
};
...
// 🚨 NOVO: Função para chamar o método de download do componente filho
...
const handleImageRotate = (newRotation) => {
  // Apenas loga a rotação. O componente editor lida com a rotação visual.
...
const handleSaveEditedNewImage = async ({ blob, isPrivate, type }) => {
 try {
  if (isEditing.value) {
   ...
   // Chamando a rota LOCAL do Nuxt Server.
   ...
   // Variável padronizada: tempFileId
   ...
   setTimeout(() => saveSuccess.value = false, 2000);
    ...
    // Tratamento de erro aprimorado para API local
    const errorMessage = err?.data?.details || err?.message || 'Erro desconhecido ao salvar a imagem temporária (Pré-Upload).';
  uploadError.value = `Falha no Pré-Upload: ${errorMessage}`;
 } finally {
  cancelEdit(); 
 }
};
